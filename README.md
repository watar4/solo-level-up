...
- **ゴールド経済**: クエスト完了・ボス討伐・貯金記録でゴールド獲得。ショップで
  消費アイテム (ポーション / 力の結晶 / 不死鳥の羽根) や武器ガチャに使う
- **DQ 風ボス戦**: ATB コマンドバトルに「どうぐ」「にげる」コマンドとかな書きの
  メッセージウィンドウ。不死鳥の羽根は戦闘不能時に自動発動
- **影の育成 (ポケモン風)**: 編成した影はボス勝利で EXP を獲得しレベルアップ。
  Lv10 で「覚醒」・Lv20 で「真」に進化して攻撃力・速度が大きく伸びる
- **討伐図鑑**: 出会った魔物と収集した影を記録。未発見はシルエット表示

## 貯金 (リアルマネー) 連携

「貯金」タブで**実際のお金 (円)** を管理できる。ゲーム内ゴールドとは別物で、
記録するとモチベ用のボーナスゴールドが少額入る (換金は双方向とも不可)。

- **手動記帳**: 預入 / 引き出し / カード利用を日付・金額・メモで記録
- **CSV インポート**: ゆうちょ銀行・楽天カードは個人向け公開 API が存在しないため
  (銀行 API は登録済み電子決済等代行業者専用)、明細 CSV の取り込みで連携する:
  - **ゆうちょダイレクト / ゆうちょ通帳アプリ** → 入出金明細 CSV (預入=貯金+、払出=貯金−)
  - **楽天e-NAVI** → 利用明細 CSV (カード支出として集計)
  - ヘッダー行を自動判別。Shift_JIS / UTF-8 両対応。同じファイルを再取り込みしても
    ハッシュで重複排除される
- **貯金目標**: 目標金額 (+任意の月間ペース) を設定すると S 級クエストとして進捗表示
- **カード予算**: 月間予算を設定し、前月を予算内で終えると報酬 (ゴールド+EXP) を請求できる

> **アップグレード時の注意**: `savingsEntries` コレクションが増えたので、既存の
> Firebase プロジェクトでは `firestore.rules` を Firebase コンソールに貼り直して
> 再公開すること (しないと貯金の保存が permission-denied になる)。
...
src/
  ...
  hooks/
    ...
    useGameData.ts              # キャラクター + クエスト購読 + 完了処理 + ゴールド経済
    useShadows.ts               # 影の購読 + 装備 + 育成 (EXP/進化)
    useSavings.ts               # 貯金台帳の購読 + 記帳 + CSV 一括取り込み
  lib/
    firestore.ts                # Firestore CRUD ラッパ
    leveling.ts                 # EXP 曲線 / ランク判定 / 日付ユーティリティ
    economy.ts                  # ゴールド報酬 / 消費アイテム / ガチャ価格
    shadowGrowth.ts             # 影のレベル・進化・戦闘力スケーリング
    savings.ts                  # 明細 CSV パーサ (ゆうちょ/楽天/汎用) + 集計
  components/
    LoginScreen.tsx
