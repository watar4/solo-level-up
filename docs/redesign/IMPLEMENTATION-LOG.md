...
## v3 C1〜C3 — AIコーチ(提案書 09 の実装)→ ❌ 撤回(2026-07-13)

> **撤回済み。** C1〜C3 を実装し main に載せたが、**利用者の主端末が iPhone(iOS Safari)**。iOS の WebKit はタブ単位メモリ上限が厳しく、最軽量 0.5B モデルでも**推論中にタブが強制リロード**され実用不可だった。段階的に対策(Web Worker 化・起動時自動ロード廃止・履歴/生成量の削減・軽量モデル誘導)を入れたが、iOS のタブOOMは JS で捕捉できないプロセス強制終了であり回避できず。「無課金かつ回数無制限」制約下では iOS 向けの LLM 代替が無く、残るルールベースのみでは価値が薄いとの判断で**コーチ機能を全撤去**した(`src/lib/coach/`・`useCoach*`・`Coach*` コンポーネント・`@mlc-ai/web-llm` 依存・デモハーネスを削除、`useGameData` の起動ナッジは v2 挙動へ復帰)。コード自体はコミット履歴(`f6b2fbf`〜`8812ae2`)に残す。以下は当時の実装記録。
